...
battery = Battery_Simple('Name', "Battery");
dcbus = DCBus_CurrentEquivalence('Name', 'Bus', 'N_inputs',1,'N_outputs',1, 'R', 0);

pmsminverter = PMSMInverter('Name', "Inverter", 'InverterType', 'VoltageDependent', 'R_2',0,'R_1',0);
...
d_opt_eta = fminbnd(@(d) (1-systemEfficiency(sys_model,d)), 0, 2);
d_opt_omega = fminbnd(@(d) -systemSpeed(sys_model,d), 0, 2);

figure
subplot(1,2,1)
p = plot(input_vector, omega_final);
datatip(p,d_opt_omega, systemSpeed(sys_model, d_opt_omega));
...
datatip(p,d_opt_eta, systemEfficiency(sys_model, d_opt_eta));
